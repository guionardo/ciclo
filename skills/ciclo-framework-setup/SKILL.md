...
- See `references/skills-packaging.md` for the **skills-in-repo pattern (ADR-004)**: `skills/<name>/` layout, how to implement `ciclo skills list/install` (resolve dir from `__dirname`, skip dotfiles, `--force`), the fake-HOME install test that never touches the real `~/.hermes/skills/`, and the pitfall of guessed commit hashes in CHANGELOG-IA entries.