...
- Installing the CLI directly from the GitHub repo: the root `package.json` (bin → `cli/bin/ciclo.js`, deps mirrored from `cli/`, `files: [cli/bin, cli/src, skills]`) makes `npm install -g guionardo/ciclo` work like `github:guionardo/ciclo` — quick install path documented in README/GUIA-DEV/ROTEIRO-REPLICACAO; update with `npm install -g guionardo/ciclo@main`. Use `npm install -g --allow-git=all guionardo/ciclo` (or set `npm config set allow-git all`) — **npm ≥ 12 blocks git deps by default** (`allow-git=none` → `EALLOWGIT refusing to fetch github:guionardo/ciclo`); the flag is accepted harmlessly on npm 9–11. Installer caveats (tested): npm ≥11 blocks dep postinstall via `allow-scripts` (informational warning; the welcome message is guaranteed by the CLI **first-run**, `~/.ciclo/first-run.json`); pnpm and bun also install the CLI but block postinstall by default; **deno is NOT supported** (CLI is CommonJS → `ReferenceError: require is not defined`).
- Setting up a new dev machine **includes installing the Hermes Agent runtime (optional but recommended for advanced features)**: `curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash` (Linux/macOS) or `iex (irm https://hermes-agent.nousresearch.com/install.ps1)` (Windows/PowerShell; WSL uses the Linux installer), then `hermes setup` / `hermes doctor`. Documented in ROTEIRO-REPLICACAO Etapa 3 and GUIA-DEV section 1.2; `verify-dev-machine.js` checks it via `hermes --version` (check 5b) and suggests the OS-specific installer. The official install commands come from the `hermes-agent` skill — do not invent them.
...
- See `references/update-check.md` for the **update check flow** (`ciclo update-check` + automatic daily check): local version vs GitHub — `gh api` (works with PRIVATE repos) preferred over anonymous HTTP; GitHub Releases changelog when available, else `CHANGELOG-IA.md` from main; cache `~/.ciclo/update-check.json` (24h TTL, null-latest cache treated as failed); `CICLO_SKIP_UPDATE_CHECK=1`/CI disables the automatic check; alias `update`, flags `--json`/`--forcar`.
